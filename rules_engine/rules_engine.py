...
from rules_generators import get_permission, glob_space, glob_container_paths, EXEC_PERMISSION
...
class Profile():
    def __init__(self, cmd, child=False):
        self._is_child = child
        self._cmd = cmd
        if(not child):
            self._child_profile = Profile(cmd, True)
    ...
    def get_child_profile(self):
        return self._child_profile
    
    def update_profile(self, line):
        ...
        if("m" not in permission):
            if(self._is_child):
                ...
                if(path.startswith(DOCKER_HOME)):
                    return
        if self._access_rules.has_key(path):
            old_permission = self._access_rules[path]
    ...
    def add_mount_rule(self, ftype, options, source, target):
        ...
        if(set(options) == set(APPARMOR_MOUNT_FLAGS_BUG)):
            mount_rule = "audit mount -> %s" % target
        else:  
            if(ftype):
...
def generate_profiles(args):
    ...
    docker_profile = Profile(args.cmd)
    ...
    with codecs.open(args.trace_file, "r", "utf-8") as trace:
        with codecs.open(errors_file, "w", "utf-8") as error:
            for line in trace:
                res = docker_profile.update_profile(line)
                if res:
                    ids_data.add(res)
                else:
    ...
    with codecs.open(parent_file_name, "w", "utf-8") as profile:
        profile.write(base_template.substitute(docker_exe=DOCKER_EXE, child=CHILD_PROFILE_NAME, docker_home=DOCKER_HOME, rules=docker_profile))
    ...
    with codecs.open(child_file_name, "w", "utf-8") as child_profile:
        child_profile.write(child_template.substitute(name=CHILD_PROFILE_NAME, parent_profile=DOCKER_EXE, rules=docker_profile.get_child_profile()))
